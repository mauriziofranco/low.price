...
c) verify into src/main/resources/application.properties if spring.datasource.* properties are correctly populated(if not, install database and prepare tables and users)
   if new development database has to be created.....
    ...
    else run the already existent db instance with the following command:
           java -classpath lib/hsqldb.jar org.hsqldb.server.Server --database.0 lowpricedb -dbname.0 lowpricedb
        
       
d) check in the pom.xml if the correct database driver is imported(via dependency tag)
e) execute spring boot app to start the backend: 
